...
private _green = "ColorGreen";
private _uncaped_eos_mkrs = all_eos_mkrs select {!(markerColor _x isEqualTo _green)};
private _debug = if (DebugEnabled isEqualTo 1) then {TRUE}else{FALSE};
...
private _nearMkrs = [_uncaped_eos_mkrs,[],{objective_pos_logic distance (markerPos _x)},"ASCEND"] call BIS_fnc_sortBy;
...
if (_debug) then {diag_log text format["Retreve Data Nearest EOS Markers : %1", _nearZones];};


private ["_rand","_selbuild","_nearBuildings","_posArray","_r","_n","_position","_clearPos"];
private _list = 1;
private _buildingNear = false;
...
while {!_buildingNear && count _nearZones > 0} do {
	...
	_nearBuildings = [markerPos _rand # 0, markerPos _rand # 1] nearObjects ["HouseBase", 75];
	if (_debug) then {diag_log text format["Retreve Data Near Buildings Iteration %1 : %2", _list, _nearBuildings];};

	if (!isNil "_nearBuildings" && {(count _nearBuildings > 0)}) then {
		_n = count _nearBuildings;
		_i = floor(random _n);
		_selbuild = (_nearBuildings # _i);
		...
		if (!isnil "_position") exitwith {
			if (_debug) then {diag_log text format["Retreve Data Near Buildings Chosen building position : %1", _position];};
			_buildingNear = true;
		};
	};
	_list = _list + 1;
...
if (_debug) then {diag_log text format["Clear Position Near Data Terminal : %1", _clearPos];};

// create data terminal
private _device = createVehicle [_type, position air_pat_pos, [], 0, "CAN_COLLIDE"]; sleep 0.03;
...
private _VarName = "Land_DataTerminal_Obj";
...
private _buildObj = nearestBuilding _bldgPos;
private _buildDir = direction _buildObj;
...
if (count(lineIntersectsObjs [(getposASL _device), [(getposASL _device select 0),(getposASL _device select 1), ((getposASL _device select 2) + 2)]]) > 1) then {
	...
	if ((count(lineIntersectsObjs [(getposASL _device), [(getposASL _device select 0),(getposASL _device select 1), ((getposASL _device select 2) + 1.8)]]) > 1) || (_alt)) then {
		...
		private _origPos = getPosATL _device;
		_device setVectorUP (surfaceNormal [_origPos # 0, _origPos # 1]);
		_bldgPos = _b_pos;
...
// create task marker
private _objmkr = createMarker ["ObjectiveMkr", _bldgPos];
...
// create defenses
private _grp = [_clearPos,10] call spawn_Op4_grp; sleep 3;
_handle=[_grp, position objective_pos_logic, 75] call BIS_fnc_taskPatrol;

private _stat_grp = [_clearPos,4,5] call spawn_Op4_StatDef;

if (_debug) then {[_grp] spawn INS_Tsk_GrpMkrs};
...
// create west task
private _tskW = "tskW_destroy_device" + _rnum;
private _tasktopicW = localize "STR_BMR_Tsk_topic_global_Retrieve_Intel";
private _taskdescW = localize "STR_BMR_Tsk_desc_global_Retrieve_Intel";
[_tskW,_tasktopicW,_taskdescW,WEST,[],"created",_bldgPos] call SHK_Taskmaster_add;
sleep 5;

// create east task
private _tskE = "tskE_defend_device" + _rnum;
private _tasktopicE = localize "STR_BMR_Tsk_topic_global_Retrieve_Intel";
private _taskdescE = localize "STR_BMR_Tsk_desc_global_Retrieve_Intel";
[_tskE,_tasktopicE,_taskdescE,EAST,[],"created",_bldgPos] call SHK_Taskmaster_add;

private _getdata = true;
while {_getdata} do
{
	if ((missionNamespace getVariable "datadownloadedby") < 3) exitWith {
		private _sideWin = missionNamespace getVariable "datadownloadedby";
