...
if (isMultiplayer) then {
	...
	if (!hasInterface && !isDedicated) then {
		...
		call compileScript ["INSfncs\hc\headless_client_fncs.sqf", false];

		0 = 0 spawn {
			if (isJIP) then {waitUntil {!isNull player}} else {waitUntil {time > 3}};
			diag_log format ["***HEADLESSCLIENT: %1 Connected HEADLESSCLIENT ID: %2", name player, clientOwner];
			...
			while {true} do {
				...
				if (_enableLogs) then {
					...
					diag_log format ["HEADLESSCLIENT FPS: %1 TIME: %2 IDCLIENT: %3 TOTAL ENEMY GROUPS: %4 TOTAL UNITS: %5", diag_fps, time, clientOwner, count _AllEnemyGroups, count allUnits];
				};
...
// Common Functions
call compileScript ["INS_definitions.sqf", false];
Remedy_SEHs_fnc = call compileScript ["INSfncs\common\Remedy_SEHs_fnc.sqf", false];
call compileScript ["INSfncs\common\commom_fncs.sqf", false];
call compileScript ["=BTC=_TK_punishment\=BTC=_tk_init.sqf", false];
if (INS_p_rev < 4 || {INS_p_rev in [6,7] && (!INS_ACE_med)}) then {
	call compileScript ["=BTC=_revive\=BTC=_revive_init.sqf", false];
}else{
	if (INS_p_rev in [4,5]) then {
		call compileScript ["=BTC=_q_revive\config.sqf", false];
	};
};
if (INS_IEDs isEqualTo 1 && !IamHC) then {
	if !(INS_op_faction in [21]) then {0 spawn {call compileScript ["EPD\Ied_Init.sqf", false]}};
};
...
if (isServer) then
{
	call compileScript ["init_server.sqf", false];
	call compileScript ["INSfncs\server\AirPatrole_Fncs.sqf", false];
	rej_fnc_bezier = compileScript ["INSfncs\server\rej_fnc_bezier.sqf", false];
...
if (hasInterface) then
{
	0 spawn {
		...
		call compileScript ["INSfncs\client\client_fncs.sqf", false];
		call compileScript ["ATM_airdrop\functions.sqf", false];
		...
		call compileScript ["init_player.sqf", false];
		call compileScript ["INSui\UI\HUD.sqf", false];
