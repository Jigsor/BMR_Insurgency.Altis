...
if !(_player in _hcEntities) then {
	...
	//{_intel = _x; [_intel,current_cache_pos] remoteExec ["fnc_jip_mp_intel",_player];} forEach intel_Build_objs;//<- old way

	//Line below is experimental new method to add action to intel. This might work better. Needs more play testing
	if (!isNil "current_cache_pos" then  {[intel_Build_objs,current_cache_pos] remoteExec ["fnc_jip_mp_intel2",_player]};
