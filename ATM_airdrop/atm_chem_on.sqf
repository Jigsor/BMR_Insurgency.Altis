...
while {(getPos _caller select 2) > 2} do {
	if(getPos _caller select 2 < 3) then {
		if (_ltcolor =="Chemlight_Red") then {_caller removeaction RedOff};
		if (_ltcolor =="Chemlight_Blue") then {_caller removeaction actionBlueOff};
		if (_ltcolor =="Chemlight_Yellow") then {_caller removeaction actionYellowOff};
		if (_ltcolor =="Chemlight_Green") then {_caller removeaction actionGreenOff};
		if (_ltcolor =="NVG_TargetC") then {_caller removeaction Iron};
	};
};